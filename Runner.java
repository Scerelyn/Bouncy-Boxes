		...
		jf.addMouseListener(new MouseAdapter(){
			...
			public void mouseReleased(MouseEvent me){
				...
				if(Double.isNaN(vectX) &&  Double.isNaN(vectY)){
					...
					if(vectY < 0){
						v.getEntityList().get( v.getEntityList().size()-1 ).setVelocity( new Velocity(1.5*Math.PI, mag) );
					} else {
					...
					if(me.getY()-45 > v.getMousePlaced().getCenterY()){ //bottom
						...
						v.getEntityList().get( v.getEntityList().size()-1 ).setVelocity( new Velocity(( (Math.PI/2)-angle )+(Math.PI/2), mag) );
					}
				} else { //right
					if (me.getY() - 45 > v.getMousePlaced().getCenterY()) { // bottom
				...
				System.out.println(v.getEntityList().get(v.getEntityList().size()-1).getVel().getMagnitude());
				System.out.println(v.getEntityList().get(v.getEntityList().size()-1).getVel().getAngle()*180/Math.PI);
